...
- **Notification Center alerts** for failed or warning jobs, with optional successful-backup summaries and a Settings test alert. The signed scheduler uses the same notification policy for scheduled runs. A reviewed recurring file-read issue can be acknowledged to silence only an exact path-and-cause match; the backup remains recorded as completed with warnings, and any new or changed cause alerts again.
