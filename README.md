...
- **Notification Center alerts** for failed or warning jobs, with optional successful-backup summaries and a Settings test alert. The signed scheduler uses the same notification policy for scheduled runs. A reviewed recurring file-read issue can be acknowledged for an exact profile, path, cause, and operation match. When every omission is known, Delta presents the run as completed with a quiet known-omissions audit note; any new or changed issue restores the warning and alerts again.
...
- **Streaming and saved backup logs** from restic stdout/stderr with source context, stable processed-file counters, clean change summaries, and a native Activity workspace that opens warning runs directly in a grouped issue review. Delta preserves restic's structured path, operation, and underlying cause, distinguishes omitted items from restic's final summary warning, and offers retry, Finder reveal, exact-path exclusion, reviewed generated-data recommendations, and repeat-alert acknowledgment. Run history is virtualized and full output is loaded from SQLite in bounded cursor pages, so large histories remain responsive.
...
- `DeltaAgent`: signed Scheduled Backups service for scheduled runs.
...
- **Packaged app verification** checks signatures, minimal hardened-runtime entitlements, Sparkle embedding, scheduler plist integrity, scheduler smoke tests, installed status-menu surface acceptance, installed scheduled-backup acceptance, installed pause/resume/cancel run-control acceptance, signed Sparkle update metadata, bundled restic/rclone versions, the restic command/flag surface Delta depends on, the external-backend parser and credential contract for every exposed restic backend family, and a real local dry-run restore that must not write files.
...
Scheduled Backups let scheduled profiles run while the main Delta window is closed. The macOS implementation is `DeltaAgent`, a signed Login Item service registered through `SMAppService`; macOS runs it as a per-user LaunchAgent under the hood. Its purpose is narrow: wake briefly after sign-in, check whether any profiles are due, start allowed backup work, and exit. In user-facing UI, Delta presents this as Scheduled Backups because LaunchAgent is the operating-system mechanism, not a product concept users should manage directly. It runs as the signed-in user, not as a privileged admin tool.
...
When an enabled scheduled profile is saved, Delta requests scheduled-backup registration automatically. If macOS still requires approval, Delta shows an action-needed scheduled-backup card on the dashboard and a detailed status in Settings. Settings also exposes Run Due Now and Open Activity actions so automatic scheduling rules can be tested and their job output can be reviewed without waiting for the next five-minute check. macOS may require manual approval in Login Items; apps cannot approve their own background items.
...
Notification Center alerts are also separate from Scheduled Backups. When enabled in Settings and allowed by macOS, Delta alerts on failed or warning jobs from either the app or the signed scheduler. Successful backup summaries are available as a separate opt-in, and Send Test Alert verifies the macOS delivery path without starting a backup. Acknowledging a structured file-read issue applies only when every issue in that run matches the same profile, path, cause, and backup operation. Delta then shows the run as completed with known omissions and removes it from Needs Attention. The original engine exit code, omitted-item list, and incomplete restore-point fact remain preserved for audit; excluding or fixing the items is still required for a technically omission-free run.
...
The release gate refuses dirty git worktrees by default so the generated app, Sparkle artifacts, release evidence, and automated-gate status match the current commit. The automated-gate status also records the approved app CDHash; doctor, evidence, and final production verification reject stale or mismatched app bundles even when the commit matches. Use `DELTA_RELEASE_ALLOW_DIRTY=1` only for local experiments that will not be used as release evidence.
...
Run the installed app bundle's status-menu surface proof directly:
...
Run the installed app bundle's mounted-volume lifecycle through a temporary APFS volume under `/Volumes`:

```sh
Scripts/run-installed-mounted-volume-acceptance.sh
```

Run the installed app bundle's REST-server lifecycle through a temporary localhost `rclone serve restic` endpoint:

```sh
Scripts/run-installed-local-rest-acceptance.sh
```

Run the installed app bundle's S3-compatible lifecycle through a temporary local rclone S3 server:

```sh
Scripts/run-installed-local-s3-acceptance.sh
```

Run the installed app bundle's SFTP lifecycle through a temporary localhost SFTP server:

```sh
Scripts/run-installed-local-sftp-acceptance.sh
```

Run the installed app bundle's rclone backend lifecycle through a temporary local rclone remote:
