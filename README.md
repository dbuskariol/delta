...
- **Packaged app verification** checks signatures, minimal hardened-runtime entitlements, Sparkle embedding, scheduler plist integrity, scheduler smoke tests, installed status-menu surface acceptance, installed scheduled-backup acceptance, installed pause/resume/cancel run-control acceptance, signed Sparkle update metadata, bundled restic/rclone versions, the restic command/flag surface Delta depends on, and a real local dry-run restore that must not write files.
