...
| Streaming and saved logs | With a large saved history, confirm Activity opens immediately, the run list scrolls smoothly, warning runs open on issue rows, All Output loads a bounded newest page, Earlier prepends the next stable page without duplicates, source context remains visible, and a running job can append output without resetting the selected run. |
