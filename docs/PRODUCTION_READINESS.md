...
Every push and pull request also runs `.github/workflows/ci.yml` on GitHub's macOS 26 runner. The workflow calls `Scripts/verify-ci.sh`, which is intentionally certificate-free: it runs tests, product-language checks, manual-matrix validators, tool verification, the local restic integration, an ad-hoc signed app build, code-signature validation, and Sparkle artifact verification. Its embedded FSKit extension is deliberately inert because Apple does not issue restricted-capability provisioning to certificate-free CI: the builder removes only that target's FSKit entitlement input, and a negative self-test rejects an entitlement-bearing or profile-bearing fixture. The local release gate remains stricter because it requires the exact FSKit entitlement and matching all-device profile, a stable Apple signing identity, and installed-app acceptance checks.
...
- installed external backend preflight for configured real-provider acceptance environments, with redacted reporting before repository initialization or backup data writes
...
- deterministic Time Machine coverage for authenticated remote generations, recovery bootstrap, path and resource bounds, cache transactions, lease loss, publication failure, service lifecycle, IPC replay, helper policy, system-controller state, and the bundled-rclone object lifecycle; this remains supporting evidence rather than installed FSKit or completed-backup acceptance
- warning-free packaged app build
- codesign validation for Delta, DeltaAgent, DeltaSecretBridge, restic, rclone, and Sparkle
- certificate-free FSKit packaging that is provably ad-hoc, teamless, unprovisioned, and missing the restricted entitlement, plus a shipping validator that requires the opposite provisioned contract
...
- installed Service Management discovery acceptance that rejects a missing helper, a mismatched `BundleProgram`, and `SMAppService.Status.notFound`
...
- bundled Login Item scheduler plist whose `BundleProgram` resolves `Contents/Resources/DeltaAgent`
...
The probe only marks machine-verifiable evidence as automated or partial evidence. It records a command-line diagnostic process's Full Disk Access observation only as nonauthoritative context because macOS may attribute privacy access to the launching shell; it never promotes that observation to evidence for the normally launched GUI app. Full Disk Access approval, macOS Login Items approval, closed-window schedule visibility, UI disconnect/reconnect behavior, menu bar visual interaction, Notification Center delivery, Sparkle install flow, and notarization remain explicit manual follow-up where a shell process would give weak or misleading evidence.
...
| Time Machine format | Using the exact stable-signed installed candidate and an isolated disposable remote store, approve File System Extensions and Background Items, create and connect a disk whose cache is smaller than its capacity, and confirm macOS identifies the case-sensitive APFS Backup volume. Complete first, no-change, and changed backups; browse and restore full and selected data; verify restored bytes; disconnect, relaunch, reconnect, and recover after removing local configuration. Measure cache and memory bounds, prove no complete sparsebundle or band payload is retained locally, exercise provider loss, upload and manifest faults, lease contention, cancellation, forced unmount, update re-registration, and safe removal, and confirm every uncertain outcome fails without false backup or eject success. Repeat the remote lifecycle against the genuine provider families required for release evidence and exercise the native UI with keyboard and VoiceOver. |
...
| New backup defaults | Change Settings > New Backup Defaults, create a new profile, and confirm schedule enabled/cadence, catch-up, battery, Low Power Mode, bandwidth, retention keep rules, prune, post-cleanup check, and cleanup cadence defaults are applied while existing profiles remain unchanged. Confirm Health Monitoring thresholds change dashboard attention timing, destination-check timing, and local/mounted destination free-space warnings without mutating profiles. |
...
| Streaming and saved logs | With a large saved history, confirm Activity opens immediately, the run list scrolls smoothly, warning runs open in grouped issue review, omitted-item count excludes restic's aggregate warning, nested restic causes and paths are visible, exact and recommended exclusions require confirmation and affect only future runs, an all-acknowledged repeat run appears as Completed with known omissions and leaves Needs Attention while preserving its raw exit code and issues, any new or changed issue restores warning attention, All Output loads a bounded newest page, Earlier prepends the next stable page without duplicates, source context remains visible, and a running job can append output without resetting the selected run. |
