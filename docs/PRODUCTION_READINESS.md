...
| New backup defaults | Change Settings > New Backup Defaults, create a new profile, and confirm catch-up, battery, Low Power Mode, bandwidth, prune, post-cleanup check, and cleanup cadence defaults are applied while existing profiles remain unchanged. Confirm the backup freshness warning threshold changes dashboard attention timing without mutating profiles. |
