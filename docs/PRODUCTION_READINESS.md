...
| Mounted SMB or NFS destination | Test at least one SMB or NFS mounted path under /Volumes, disconnect it, confirm Delta reports destination unavailable without invoking restic, reconnect it, and confirm backup resumes. |
...
| Notifications | Enable job alerts in Settings, allow macOS notification permission, trigger one warning or failed job from the app and one scheduled helper job, and confirm successful-backup summaries only appear when the separate success setting is enabled. |
