...
| Streaming and saved logs | With a large saved history, confirm Activity opens immediately, the run list scrolls smoothly, warning runs open in grouped issue review, omitted-item count excludes restic's aggregate warning, nested restic causes and paths are visible, exact and recommended exclusions require confirmation and affect only future runs, acknowledgment retains warning history while suppressing an exact repeated notification, All Output loads a bounded newest page, Earlier prepends the next stable page without duplicates, source context remains visible, and a running job can append output without resetting the selected run. |
