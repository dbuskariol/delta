...
- clean first-registration Time Machine system-support acceptance for the exact notarized `/Applications/Delta.app`: both public registrations must start unregistered, administrator approval must make them enabled, and the running privileged helper must return the exact embedded Security code hash before candidate-bound evidence is recorded and both registrations are removed through public APIs
