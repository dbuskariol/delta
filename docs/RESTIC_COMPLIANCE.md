...
For backup JSON `error` messages, Delta parses restic's nested `error.message`, `during`, and `item` fields into structured persisted issue metadata. The Activity count represents omitted items rather than counting restic's final aggregate warning as another file. Issues are grouped by classified cause while retaining their exact redacted path, operation, and message. Restic exit code `3` remains a warning because the created snapshot is incomplete; acknowledgment can suppress a repeated notification but cannot rewrite that outcome.
