...
For backup JSON `error` messages, Delta parses restic's nested `error.message`, `during`, and `item` fields into structured persisted issue metadata. The Activity count represents omitted items rather than counting restic's final aggregate warning as another file. Issues are grouped by classified cause while retaining their exact redacted path, operation, and message. Restic exit code `3` remains stored as the technical result because the created snapshot omitted files. When every issue exactly matches the user's acknowledged baseline, Delta's user-facing outcome becomes Completed with a known-omissions audit note and no attention alert. The raw exit code, issue list, and restore-point completeness are never rewritten; any new path or changed cause restores the warning state.

Activity exclusions reuse the profile's normal `--exclude` pipeline. Exact-path actions escape restic glob metacharacters, and batch recommendations are limited to explicit generated-data classifiers. Unknown paths are never automatically excluded. Existing snapshots and historical job status are not modified when coverage changes.
...
When a user saves an enabled scheduled profile, Delta requests Scheduled Backups registration through `SMAppService` if the helper is not already registered. The launch-agent property list lives under `Contents/Library/LaunchAgents` and its `BundleProgram` resolves the signed helper at `Contents/Resources/DeltaAgent`, following Service Management's embedded-helper layout. If macOS reports that Login Items approval is still required, Delta records the schedule and surfaces the approval action instead of silently leaving scheduled backups inert.
...
`ResticRunner` streams stdout and stderr while the process is running and preserves UTF-8 characters that arrive across pipe-read boundaries. The coordinator records start, streamed output, and finish lines as per-job SQLite log entries and forwards current events to the app's active-run presentation. Activity reads bounded pages from the durable log store, which gives app-owned and scheduler-owned jobs the same source of truth without retaining or rendering the full stream in memory. Individual streamed lines are bounded; an oversized line is replaced by a safe omission marker and later lines continue to flow.

Final process capture is command-aware. Operational commands retain an 8 MiB stdout tail and a 4 MiB stderr tail so final summaries and failure diagnostics survive without allowing a noisy process to exhaust memory. Commands whose output must be parsed as a complete data set fail closed instead of using partial JSON: snapshots and restore browsing allow up to 64 MiB, while repository-key listing allows up to 16 MiB. Truncation state remains explicit on the run result. Status JSON is additionally persisted as a compact structured progress snapshot on the running job row, so the app can reconstruct progress after relaunch or when a scheduled backup is owned by `DeltaAgent`.

When a scheduled backup is started by `DeltaAgent`, the app polls the same SQLite job/log state so the dashboard, Activity page, and menu bar still show the active operation. Restic JSON status/error lines are formatted into readable messages before durable storage, and backup item errors retain typed issue metadata for review without retaining unredacted raw secrets. Retention arrays are reduced to kept/removed counts, and unknown structured arrays are summarized by item count instead of echoing snapshot hostnames, usernames, tags, or other unneeded repository metadata into Activity.
...
- production external acceptance evidence verification for a real mounted SMB/NFS destination, real SFTP destination, and real S3-compatible destination, rejecting localhost harness reports and stale reports whose app path, executable path, bundled backup-engine path, app CDHash, lifecycle counts, or git commit do not match the release candidate
- a hermetic external-evidence verifier self-test that proves stale app paths, stale CDHashes, localhost evidence, and thin lifecycle evidence fail the production gate
