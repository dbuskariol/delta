...
if [[ ! -d "$APP_PATH" ]]; then
  ...
  if [[ "$installed_preferences_status" -eq 0 ]]; then
    installed_preferences_evidence="Installed preferences acceptance passed: the signed app verified the shared Settings surface contract, required categories, compact status summary, recommended defaults, invalid preference normalization, custom backup defaults persisted to a new profile, custom restore defaults, diagnostic preference summaries, and preference restoration. $installed_preferences_output"
  else
  ...
  if [[ "$automated_gate_status" == "Passed" && "$automated_gate_commit" == "$git_commit" ]]; then
    ...
    if [[ "$installed_preferences_status" -eq 0 ]]; then
      append_row "new_backup_defaults" "$(item_area new_backup_defaults)" "Partial" "$installed_preferences_evidence Automated release gate also passed backup-default preference, health-threshold normalization, destination free-space warning, source-access dashboard health evaluation, and schedule policy tests for commit $git_commit." "Use the Settings UI to change defaults and confirm newly-created UI profiles inherit them without mutating existing profiles. Confirm Health Monitoring thresholds change dashboard attention timing, destination-check timing, and local/mounted destination free-space warnings without mutating profiles."
  ...
  if [[ "$authority" != Developer\ ID\ Application:* ]]; then
    append_row "developer_id_notarization" "$(item_area developer_id_notarization)" "Manual Required" "Current local build is signed as '${authority:-Unknown}', not Developer ID. Developer ID signature, notarization submission, stapling, and Gatekeeper assessment are external release steps." "Sign with Developer ID, submit with notarytool, staple, archive notary logs, and rerun production readiness."
  else
    notarization_output="$(run_capture notarization /bin/sh -c "/usr/bin/xcrun stapler validate '$APP_PATH' >/dev/null 2>&1 && /usr/sbin/spctl --assess --type execute '$APP_PATH' >/dev/null 2>&1")"
    ...
    if [[ "$notarization_status" -eq 0 ]]; then
      ...
      append_row "developer_id_notarization" "$(item_area developer_id_notarization)" "Failed" "Developer ID signature is present, but stapled ticket or Gatekeeper assessment is missing. $notarization_output" "Submit with notarytool, staple, and rerun production readiness."
    fi
  fi
fi
