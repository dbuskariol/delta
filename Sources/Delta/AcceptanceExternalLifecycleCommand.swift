...
enum AcceptanceExternalLifecycleCommand {
    static func run(
        ...
        if kind == .s3 {
            credentialReferences = try credentialResolver.saveCredentials(
                try s3Credentials(from: environment),
    ...
    private static func s3Credentials(from environment: [String: String]) throws -> [String: String] {
        let accessKey = try requiredEnvironment("AWS_ACCESS_KEY_ID", environment: environment)
        let secretKey = try requiredEnvironment("AWS_SECRET_ACCESS_KEY", environment: environment)
        var credentials = [
            "AWS_ACCESS_KEY_ID": accessKey,
            "AWS_SECRET_ACCESS_KEY": secretKey
        ]
        if let sessionToken = environment["AWS_SESSION_TOKEN"], !sessionToken.isEmpty {
...
private enum AcceptanceExternalKind: String {
    ...
    init(environmentValue: String?) throws {
        guard
            ...
            throw AcceptanceExternalLifecycleError.validationFailed("DELTA_EXTERNAL_ACCEPTANCE_KIND must be mounted, sftp, or s3.")
        }
        self = kind
    }
